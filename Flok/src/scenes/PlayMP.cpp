...
void Input() {
  ...
  if (pause) {
    ...
    if (IsKeyPressed(KEY_UP)) {
      PlayerClass2::Push({0, -1}, k_PlayerUpwardPush + PlayerClass2::GetSpeed());
    }
  }
}
...
void PlayMP::Play() {
  ...
  int x = PlayerClass::GetBoundingBox().x - PlayerClass::GetBoundingBox().width * 0.5f;
  int x2 = PlayerClass2::GetBoundingBox().x + PlayerClass2::GetBoundingBox().width * 0.5f;

  PlayerClass::SetPosition(x, PlayerClass::GetBoundingBox().y);
  PlayerClass2::SetPosition(x2, PlayerClass2::GetBoundingBox().y);
