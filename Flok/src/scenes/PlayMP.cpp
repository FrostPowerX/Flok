...
bool CheckForPlayerWallCollision(const std::list<WallType*>& Walls) {
  ...
  for (const auto Wall : Walls) {
    if (Wall) {
      ...
      Rectangle const Bb2 = GetBoundingBoxPlayer(f_Player2);
      Collided = (Collisions::IsRectRect(
                  ...
                  Collisions::IsRectRect(
                      Bb2, {.x = Wall->f_Position, .y = 0, .width = Wall->f_WallWidth, .height = Wall->f_GapStart}) ||
                  Collisions::IsRectRect(Bb2, {.x = Wall->f_Position,
                                               .y = Wall->f_GapStart + Wall->f_GapSize,
                                               .width = Wall->f_WallWidth,
                                               .height = g_ScreenHeight - (Wall->f_GapStart + Wall->f_GapSize)}));
    }
...
void Play() {
  ...
  float x2 = GetBoundingBoxPlayer(f_Player2).x + GetBoundingBoxPlayer(f_Player2).width * 0.5f;

  SetPositionPlayer(f_Player1, x, GetBoundingBoxPlayer(f_Player1).y);
  SetPositionPlayer(f_Player2, x2, GetBoundingBoxPlayer(f_Player2).y);
