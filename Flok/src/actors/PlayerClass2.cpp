...
PlayerClass2::PlayerType Player;
} // namespace

void PlayerClass2::Init() {
...
void PlayerClass2::Unload() {
  if (IsTextureReady(Player.f_Sprite)) {
    UnloadTexture(Player.f_Sprite);
  }
}

void PlayerClass2::Push(const Vector2& Direction, float Force) {
...
Rectangle PlayerClass2::GetBoundingBox() {
  return Player.f_BoundingBox;
}

void PlayerClass2::SetPosition(const int x, const int y) {
  Player.f_BoundingBox.x = x;
  Player.f_BoundingBox.y = y;
}

void PlayerClass2::MovePlayer(const float Height) {
  Player.f_BoundingBox.y += Height;
}

float PlayerClass2::GetSpeed() {
